...
function showSplitShapeDialog() {
  ...
  const selectedShapes = selection.getPageElementRange() ? 
                        selection.getPageElementRange().getPageElements().filter(
                          element => element.getPageElementType() === SlidesApp.PageElementType.SHAPE
                        ) : [];
  
  if (selectedShapes.length !== 1) {
    ui.alert('Error', 'Please select exactly one shape to split.', ui.ButtonSet.OK);
    return;
  }
...
function splitSelectedShape(rows, columns, gap) {
  try {
    ...
    console.log('Position: Left ' + originalShape.getLeft() + ', Top ' + originalShape.getTop());
    console.log('Size: Width ' + originalShape.getWidth() + ', Height ' + originalShape.getHeight());
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < columns; col++) {
        ...
        const newShape = slide.insertShape(originalShapeType, left, top, cellWidth, cellHeight);
    ...
    console.log('Successfully created ' + newShapes.length + ' shapes in a ' + rows + 'x' + columns + ' grid with ' + gap + 'pt gaps');
...
function copyShapeStyle(sourceShape, targetShape) {
  try {
    ...
    if (sourceFill.getType() === SlidesApp.FillType.SOLID) {
      targetFill.setSolidFill(sourceFill.getSolidFill().getColor(), sourceFill.getSolidFill().getAlpha());
    }
    ...
    if (borderFill.getType() === SlidesApp.FillType.SOLID) {
      targetBorder.setSolidFill(borderFill.getSolidFill().getColor(), borderFill.getSolidFill().getAlpha());
    }
