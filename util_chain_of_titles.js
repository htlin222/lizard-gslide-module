...
function copyPreviousTitleText() {
  ...
  const currentIndex = slides.findIndex(slide => slide.getObjectId() === currentSlide.getObjectId());
  ...
  for (let element of pageElements) {
    if (element.getPageElementType() === SlidesApp.PageElementType.SHAPE && element.getTitle() === "PREVIOUS_TITLE") {
  ...
  const newShape = currentSlide.insertTextBox(titleText, 24, 18, 200, 25);
...
function createNextSlideWithCurrentTitle() {
  const presentation = SlidesApp.getActivePresentation();
  const slides = presentation.getSlides();
  const currentPageId = presentation.getSelection().getCurrentPage().getObjectId();

  // Find current slide index
  const currentIndex = slides.findIndex(slide => slide.getObjectId() === currentPageId);
